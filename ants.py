        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    #add ant to container ant already in place
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant):
                    #add container ant around ant already in place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while len(check_place.bees) == 0 and check_place.entrance != hive:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        # BEGIN Problem 3B
        check_place = self.place.entrance.entrance.entrance.entrance.entrance
        while len(check_place.bees) == 0 and check_place.entrance != hive:
            check_place = check_place.entrance
        return random_or_none(check_place.bees)
        # END Problem 3B
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        while len(check_place.bees) == 0 and check_place.entrance != hive and i < self.max_range:
            check_place = check_place.entrance
            i += 1
        return random_or_none(check_place.bees)
    implemented = True   # Change to True to view in the GUI
...
class HungryAnt(Ant):
    ...
    #def __init__(self, armor=1, time_to_digest=3, digesting=0):
        # BEGIN Problem 6B
    def __init__(self):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    impost = False
    implemented = True
    buffed_ants = []   # Change to True to view in the GUI
    # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.impost:
            ...
            while prev_place:
                if prev_place.ant and not prev_place.ant.container and prev_place.ant not in QueenAnt.buffed_ants:
                    prev_place.ant.damage = 2*prev_place.ant.damage
                    QueenAnt.buffed_ants.append(prev_place.ant)
                elif prev_place.ant and prev_place.ant.container:
                    if prev_place.ant not in QueenAnt.buffed_ants:
                        prev_place.ant.damage = 2*prev_place.ant.damage
                        QueenAnt.buffed_ants.append(prev_place.ant)
                    if prev_place.ant.ant != None and prev_place.ant.ant not in QueenAnt.buffed_ants:
                        prev_place.ant.ant.damage *=2
                        QueenAnt.buffed_ants.append(prev_place.ant.ant)
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def effect_timer(timer):
        def slow_action(colony):
            nonlocal action, timer
            if timer > 0:
                timer -= 1
                if colony.time % 2 == 0:
...
def apply_effect(effect, bee, duration):
    ...
    #def call_counter(calls, effect, bee, duration, og_action):
    #    calls += 1
    #    def applier(calls):
    #        nonlocal effect, bee, duration, og_action
    #        if calls <= duration:
    #            bee.action = effect(bee.action)
    #            return bee.action
    #        else:
    #            bee.action = og_action
    #            return bee.action
    #    return applier
    #return call_counter
